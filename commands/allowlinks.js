exports.run = async (client, message, args) => {
  ...
  if(channel.topic.includes('{deletemessages}'))
    return message.channel.send(`:warning: Messages sent in ${sameChannel ? 'this' : 'that'} channel are already moved`)

  await channel.edit({
    topic: channel.topic + ' {deletemessages}'
  },
  `Allowing links: action taken by ${message.author.tag}`
  )

  message.channel.send(`:white_check_mark: Messages will now be moved ${sameChannel ? 'this' : 'that'} channel`)

}

exports.help = {
  name: 'allowlinks',
  info: 'Allows links to be sent in a channel',
  usage: '[channel]',
  unlisted: false,
}
